...
- [Быстрый старт](http://docs.angularjs.org/tutorial/ "angularjs быстрый старт") (пошаговая разработка простого приложения)
- [Руководство разработчика](http://docs.angularjs.org/guide/ "Руководство AngularJs") и [API](http://docs.angularjs.org/api "AngularJs API")
...
Преимущества использования AngularJS(от [сюда](http://habrahabr.ru/post/142590/)):
...
- [AngularJS Hello World](http://www.youtube.com/watch?v=uFTFsKmkQnQ) ( 4 минуты )
- [AngularJS Tutorial](http://www.youtube.com/watch?v=WuiHuZq_cg4) ( 12минут )
- [AngularJS - Routes and Partials](http://www.youtube.com/watch?v=zogrnQjHZAM) ( 14 минут )
...
- [ng-inspector](http://ng-inspector.org/) - расширение для Chrome направленное на анализ scope
...
Двойная фигурная кавычка используется для шаблонов: внутри можно указать переменную, выражение(подробнее [в части о шаблонах](http://stepansuvorov.com/blog/2013/06/start-with-angularjs-part2/)).
...
1. создается специальный объект Инжектор([$injector](http://docs.angularjs.org/api/AUTO.$injector)), по средствам которого получаются все остальные сущности
2. Инжектор создает [$rootScope](http://docs.angularjs.org/api/ng.$rootScope), что является контекстом для модели нашего приложения
...
[ng-repeat](http://docs.angularjs.org/api/ng.directive:ngRepeat) - директива, которая запускает перебор коллекции(аналог forEach)
...
\- в директиве [ng-controller](http://docs.angularjs.org/api/ng.directive:ngController) мы указываем, какой контролер должен обрабатывать данную область.
...
В нашем случае мы просто заносим в [$scope](http://docs.angularjs.org/api/ng.$rootScope.Scope) коллекцию объектов, чтобы потом ее отобразить во вью.

**$scope, $rootScope**

В AngularJS **$scope** и **$rootScope** - это ключевые сущности для передачи данных между моделью и отображением посредством контроллера. При этом $rootScope - это корневой scope, к которому имеют доступ все, а  $scope - scope определенного контроллера. Т.е. в нашем примере мы занесли данные только в scope нашего контроллера(_PhoneListCtrl_), а если бы мы хотели, чтобы другие контроллеры тоже получили доступ, то нам необходимо было использовать $rootScope. Более подробно можно почитать [тут](http://docs.angularjs.org/api/ng.$rootScope.Scope). ( _также в тему_: [Передача данных между сущностями AngularJS](http://stepansuvorov.com/blog/2014/09/angularjs-data-transfer/) )
...
Кто разбирался с [Jasmine](http://stepansuvorov.com/blog/2012/10/jasmine-%D0%B8-%D1%8E%D0%BD%D0%B8%D1%82-%D1%82%D0%B5%D1%81%D1%82%D1%8B/), нет ничего особо сложного: в данном тесте мы проверяем контроллер и хотим убедиться что он занес в коллекцию 3 записи.

**Запуск тестов используя [karma](http://karma-runner.github.io/).**
...
**UPD:** [AngularJs Basics](http://www.angularbasics.co.uk/ "на английском ") - прекрасно структурированный сайт документации
